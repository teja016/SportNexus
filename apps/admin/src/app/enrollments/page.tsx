...
const STATUS_TABS = ['ALL', 'PENDING', 'CONFIRMED', 'ACTIVE', 'CANCELLED', 'COMPLETED']
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b sticky top-0 bg-white z-10">
          <div>
            ...
            <p className="font-mono font-bold text-navy text-lg">#{enrollment.id?.slice(-8).toUpperCase()}</p>
          </div>
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex justify-between items-center px-4 py-3 gap-4">
      ...
      <span className={`text-sm font-semibold text-right ${highlight ? 'text-primary text-base' : 'text-gray-800'}`}>{value}</span>
    </div>
  )
}
...
export default function EnrollmentsPage() {
  ...
  const filtered = activeStatus === 'ALL'
    ? enrollments
    : enrollments.filter((e) => e.status === activeStatus)

  const counts: Record<string, number> = {}
  STATUS_TABS.forEach((s) => {
    counts[s] = s === 'ALL' ? enrollments.length : enrollments.filter((e) => e.status === s).length
  })
      ...
      <PageHeader
        ...
        subtitle={
          <span className="flex items-center gap-3">
            ...
            <button onClick={() => load()} className="text-xs text-primary font-semibold hover:underline">↻ Refresh</button>
          </span>
        }
      />
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100">
                {['Ref', 'Student', 'Academy', 'Program', 'Duration', 'Transport', 'Status', 'Enrolled On', ''].map((h) => (
            ...
            <tbody>
              {filtered.map((e) => (
                ...
                >
                  ...
                  <td className="px-5 py-3.5">{e.transportOpted ? <span className="text-teal-600 font-semibold text-xs">Yes</span> : <span className="text-gray-400 text-xs">No</span>}</td>
                  <td className="px-5 py-3.5"><StatusBadge status={e.status} /></td>
                  <td className="px-5 py-3.5 text-gray-400 text-xs">{e.enrolledAt ? new Date(e.enrolledAt).toLocaleDateString('en-IN') : '—'}</td>
