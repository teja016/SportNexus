...
import {
  ...
  Modal, Switch, StyleSheet, Image, Dimensions,
} from 'react-native'
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.88}>
      {photo ? (
        ...
        <View style={[styles.cardImg, { backgroundColor: sportColor + '15', alignItems: 'center', justifyContent: 'center' }]}>
          <Text style={{ fontSize: 30 }}>{SPORT_ICONS[sport] ?? '🏅'}</Text>
        </View>
      )}
      ...
      <View style={styles.cardBody}>
        ...
        {minFee > 0 && <Text style={styles.cardFee}>₹{minFee}<Text style={styles.cardFeeSub}>/mo</Text></Text>}
      </View>
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.searchRow}>
          ...
          >
            <Ionicons name="options-outline" size={20} color={activeFilters > 0 ? '#fff' : Colors.primary} />
            {activeFilters > 0 && (
        ...
        <View style={styles.sortRow}>
          {SORT_OPTIONS.map((s) => (
            ...
            >
              <Ionicons name={s.icon} size={13} color={sortBy === s.key ? '#fff' : Colors.textSecondary} />
            ...
            {SPORT_TYPES.map((s) => {
              ...
              return (
                ...
                >
                  ...
                  <Text style={[styles.filterChipText, active && { color: '#fff' }]}>{s}</Text>
                </TouchableOpacity>
              )
            })}
          </View>
...
const styles = StyleSheet.create({
  ...
  header:    { backgroundColor: Colors.surface, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: Colors.border, ...Shadow.xs },
  searchRow: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 16, paddingTop: 56, paddingBottom: 10 },
  searchBox: {
  ...
  filterBtn:      { width: 46, height: 46, borderRadius: BorderRadius.md, backgroundColor: Colors.tealXLight, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: Colors.primary + '30' },
  filterBtnActive:{ backgroundColor: Colors.primary, borderColor: Colors.primary },
  filterDot:      { position: 'absolute', top: -4, right: -4, width: 16, height: 16, borderRadius: 8, backgroundColor: Colors.danger, alignItems: 'center', justifyContent: 'center', borderWidth: 1.5, borderColor: '#fff' },
  filterDotText:  { fontSize: 9, color: '#fff', fontWeight: FontWeight.bold },

  sortRow:       { flexDirection: 'row', gap: 8, paddingHorizontal: 16 },
  sortPill:      { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingVertical: 7, borderRadius: BorderRadius.full, backgroundColor: Colors.background, borderWidth: 1.5, borderColor: Colors.border },
  sortPillActive:{ backgroundColor: Colors.primary, borderColor: Colors.primary },
  sortText:      { fontSize: FontSize.xs, color: Colors.textSecondary, fontWeight: FontWeight.semibold },
  ...
  emptyIcon:       { width: 72, height: 72, borderRadius: 36, backgroundColor: Colors.borderLight, alignItems: 'center', justifyContent: 'center', marginBottom: 4 },
  ...
  applyBtn:    { backgroundColor: Colors.primary, paddingVertical: 16, borderRadius: BorderRadius.lg, alignItems: 'center', marginTop: 24, marginBottom: 8, shadowColor: Colors.primary, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.25, shadowRadius: 16, elevation: 8 },
